...
#   #####################################
#
#    ODEPACK.py (see https://github.com/hplgit/odespy). 
#    Modified to save roots in 'lsodar'. 
#    Sections changed are marked with ##################
#    Please download the original package and replace odepack.py with this file.
#
#########################################
...
class Odepack(Solver):
    ...
    *Note*: For large ODE systems the user is encouraged that users provide
    ...
    def print_roots(self, jroot, t_current, u_current):
        '''Roots at current T for some constraint functions. '''
    ...
    ##################
    def save_roots(self, jroot, t_current, u_current,  t_before, u_before):
        ''' added function to not only print but also save roots for i-th function g '''
        g, ng = self.g_f77, self.ng
        if hasattr(g, '_cpointer'):  # ng is required if g is in Fortran
            value = g(t_current, u_current, ng=ng)
        else:
            value = g(t_current, u_current)
        output = [[] for i in range(self.ng)]
        for i in range(ng):
            try:
              value = value[i] 
            except IndexError:
              value = value  # value is g evaluated at current time
            value_before = g(t_before, u_before)[i]  # value_before is g evaluated one step earlier.
            condition = value_before>0 if i==0 else value_before<0  # condition to plot only one direction of poincare section
            if jroot[i] and condition: # found root for i-th constraint equation and going through from one direction (condition)
                value_ith = value
                output[i] = [u_current, t_current, i]
            else:
                output[i] = []
        return output 
    #################
    ...
    def solve(self, time_points, terminate=None):
        ...
        #############################
        if solver_name == 'dlsodar':
          self.roots = [[] for i in range(self.ng)] # empty list for each constraint function i (list of lists)
          self.roots_time = [[] for i in range(self.ng)]
        roots_found = 0
        #############################
        while not self.finished:
            ...
            if nstop == step_no:    # successful
                self.finished = True
                tried = 0
            if istate == 3:         # roots found for constraint equations
                #################################
                # self.print_roots(iinfo[17:], self.t[nstop-1], u[nstop-1])
                self.output = self.save_roots(iinfo[17:], self.t[nstop-1], u[nstop-1], self.t[nstop-2], u[nstop-2])
                for i in range(self.ng):
                  if self.output[i]:
                    roots_found = 1
                    self.roots[i].append(self.output[i][0])  # save roots (u at time where g=0)
                    self.roots_time[i].append(self.output[i][1])  # save timepoints where g=0
                #################################
                tried = 0
            elif istate == 2:
                self.finished = True
        ...
        ##################################
        # return roots and roots_time if the method is lsodar (lsoda with root finding)
        if solver_name == 'dlsodar':
          return self.u, self.t, self.roots, self.roots_time
        else:
